...
>
> **Status:** v0.4 DRAFT — M2-IG.1 batch 1 (registry + secrets ADRs; M2 split into M2-IB PARKED / M2-IG ACTIVE). Will be edited every time an artifact is added, retired, or moved.
>
...
| **KB-10** | [`docs/decisions/DECISIONS.md`](./docs/decisions/DECISIONS.md) | DRAFT v0.7 (2026-04-27) | ADR-001..029 ACCEPTED; ADR-030..035 PROPOSED. M2-IB substrate batch (committed at `M2-substrate-IB.checkpoint`): ADR-030 (per-archetype btest interpreter dispatch), ADR-031 (token-bucket rate limiter shape; algorithm broker-agnostic, IB defaults), ADR-032 (instrument resolution: blive.Instrument ↔ IB Contract / ConID; pairs with DD-7), ADR-033 (AccountUpdate event shape, 30-s diff-suppressed cadence; pairs with DD-2). M2-IG.1 batch 1 (this commit): ADR-034 (multi-broker registry pattern; extends ADR-004 hexagonal contract for N>2 brokers; load-bearing for IG bridge), ADR-035 (secrets handling discipline; ~/.blive/secrets/{broker}.env + redaction list). | Claude record, Oleg approve | continuous |
...
7. **M2-IG.1 batch 1 substrate** — current commit: ADR-034 (multi-broker registry pattern, extends ADR-004), ADR-035 (secrets handling discipline `~/.blive/secrets/{broker}.env`), TASK_REGISTRY v0.2 (M2 split into M2-IB PARKED + M2-IG ACTIVE; G3 → G3-IB/G3-IG). Awaiting operator review before flipping ADR-034 / ADR-035 status to ACCEPTED.
8. **M2-IG.1 batch 2 substrate (next)** — IG-specific ADRs ADR-036 (IG wire-level driver: roll-our-own httpx + asyncio Lightstreamer per operator), ADR-037 (`Instrument.tradability` field), ADR-038 (IG rate-limit defaults; parameterises ADR-031), ADR-039 (Phase 1 strategy under IG bridge: CAC 40 CFD; ADR-021 PAUSED). New substrate: KB-16 (IG capability matrix), KB-17 (IG pacing spec), DD-8 (IG instrument dictionary). Plus DD-3 / DD-1 / INV-6 / KB-8 amendments.
9. **M2-IG.2 cross-cutting infra code (post all-substrate-ACCEPTED)** — `blive.adapters.shared.rate_limiter`, `blive.adapters.shared.credentials`, `blive.runtime.broker_registry`, `secrets/ig.env.example`, `.gitignore` rule, import-linter contract amendment.
10. **M2-IG.3, M2-IG.4, M2-IG.5** — IG read-side adapter, write-side adapter, strategy run + retro. See [TASK_REGISTRY M2-IG](./TASK_REGISTRY.md) sub-milestones.
