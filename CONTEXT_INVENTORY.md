...
| 5. Code | `src/blive/` | DRAFT v0.4 (2026-04-27) | continuous | M0+M1: `domain/{types,events,order_fsm,ports,positions}.py`; `adapters/{paper.{broker,market_data}, memory.{persistence,bus}, clock.{wall,sim}, alert.log}`; `strategy/{config,loader}.py`; `sizing/sizer.py`; `risk/checks.py`; `runtime/paper_pipeline.py`. **M2-IG.2:** `adapters/shared/{rate_limiter,credentials}.py` (broker-agnostic token bucket per ADR-031+ADR-038; env-var/dotenv credentials loader per ADR-035 with redaction-list helper). `domain/types.py` v0.2 added `Tradability` literal alias + `Instrument.tradability` field per ADR-037. |
| 6. Tests | `tests/` | DRAFT v0.4 (2026-04-27) | continuous | 213 tests green. M1 added `unit/{strategy,sizing,risk,runtime}/*`, `unit/adapters/{paper/test_market_data,paper/test_paper_broker_replace,alert/test_log_alert}/*`, `contracts/test_btest_imports.py`. **M2-IG.2:** `unit/adapters/shared/test_rate_limiter.py` (19 tests including G3-IB and G3-IG throttle tests under SimClock); `unit/adapters/shared/test_credentials.py` (16 tests covering schema validation, env-var override, dotenv parsing, missing-required raises, redaction-list union); `unit/domain/test_types.py` +3 tests for `Instrument.tradability` (default, validation, identity widening). |
