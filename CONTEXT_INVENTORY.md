...
| 6. Tests | `tests/` | DRAFT v1.5 (2026-05-01) | continuous | 449 tests green (was 416 at M2-IB.3a-resolved). M0/M1/M2-IG/M2-IB.2/M2-IB.3a modules unchanged. **M2-IB.3b-i + .3b-i-timer additions:** `unit/adapters/ib/test_broker.py` has 33 tests for IBBroker read side — connect/disconnect/idempotency, ConnectionStatus emission, accountValues seed + post-connect event-push merging, account_snapshot composition + base-currency inference + cash_by_ccy + leverage, positions parsing (primaryExchange-preferring, unparseable-skip), open_orders parsing (LMT happy path, unsupported orderType skip), rate-limit acquire on positions/open_orders, write methods raise NotImplementedError. Plus 9 timer tests: baseline emission on first tick, diff-suppress (no emit on identical state), above-threshold emit (equity / cash_by_ccy), below-threshold skip (currency 0.01 / leverage 0.001), task cancellation on disconnect. Uses real `eventkit.Event` for `accountValueEvent`. |
