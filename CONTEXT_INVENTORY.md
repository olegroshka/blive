...
>
> **Status:** v0.5 DRAFT — M2-IG.1 batch 2 (IG-specific ADRs 036–039 + KB-16/17 + DD-8). Will be edited every time an artifact is added, retired, or moved.
>
...
| **KB-10** | [`docs/decisions/DECISIONS.md`](./docs/decisions/DECISIONS.md) | DRAFT v0.8 (2026-04-27) | ADR-001..029 ACCEPTED; ADR-030..039 PROPOSED. M2-IB substrate batch (`M2-substrate-IB.checkpoint`): ADR-030..033. M2-IG.1 batch 1 (`M2-IG.1-batch1`): ADR-034 (multi-broker registry pattern), ADR-035 (secrets handling discipline). M2-IG.1 batch 2 (current): ADR-036 (IG wire-level driver: roll-our-own httpx + asyncio Lightstreamer per operator), ADR-037 (`Instrument.tradability` field — backward-compatible spot/cfd/spread_bet discriminator; scopes ADR-027 to spot only), ADR-038 (IG rate-limit defaults — parameterises ADR-031 with named-bucket config; 30/60/40 per minute + 40 concurrent Lightstreamer subscriptions), ADR-039 (Phase 1 strategy under IG bridge — CAC 40 CFD; ADR-021 PAUSED; new envelope: directional alignment + characterised < 100 bps over 5-day run). | Claude record, Oleg approve | continuous |
...
9. **M2-IG.2 cross-cutting infra code (post all-substrate-ACCEPTED)** — `blive.adapters.shared.rate_limiter` ([ADR-031](./docs/decisions/DECISIONS.md#adr-031--token-bucket-rate-limiter-shape-for-ib-adapters)+[ADR-038](./docs/decisions/DECISIONS.md#adr-038--ig-rate-limit-defaults-parameterise-adr-031)), `blive.adapters.shared.credentials` ([ADR-035](./docs/decisions/DECISIONS.md#adr-035--secrets-handling-discipline-blivesecrets)), `blive.runtime.broker_registry` ([ADR-034](./docs/decisions/DECISIONS.md#adr-034--multi-broker-registry-pattern-extends-adr-004)), DD-1 amendment for `tradability` ([ADR-037](./docs/decisions/DECISIONS.md#adr-037--instrumenttradability-field-spot--cfd--spread_bet)), DD-3 amendment for top-level `broker` field, INV-6 amendments for IG rows + new `import-linter` contract.
