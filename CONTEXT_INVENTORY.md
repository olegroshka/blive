...
| 5. Code | `src/blive/` | DRAFT v0.8 (2026-04-28) | continuous | M0+M1: `domain/{types,events,order_fsm,ports,positions}.py`; `adapters/{paper.{broker,market_data}, memory.{persistence,bus}, clock.{wall,sim}, alert.log}`; `strategy/{config,loader}.py`; `sizing/sizer.py`; `risk/checks.py`; `runtime/paper_pipeline.py`. **M2-IG.2:** `adapters/shared/{rate_limiter,credentials}.py`; `runtime/broker_registry.py`; `domain/types.py` v0.2 (Tradability + Instrument.tradability); `strategy/config.py` v0.2 (LiveStrategyConfig.broker). **M2-IG.3 in flight:** `adapters/ig/{__init__,credentials}.py` (IG_SCHEMA + typed IGCredentials); `adapters/ig/client.py` (REST core per ADR-036 — IGClient + 7-class typed exception hierarchy + KB-17 §6 errorCode classifier); `adapters/ig/instrument_resolver.py` (Instrument↔IG epic per DD-8 + ADR-037 — IGInstrumentResolver with epic-guess-then-search fallback, in-process cache, IGMarketMetadata dataclass, precision_for lookup). |
| 6. Tests | `tests/` | DRAFT v0.8 (2026-04-28) | continuous | 270 tests green. M1 added `unit/{strategy,sizing,risk,runtime}/*`, `unit/adapters/{paper/test_market_data,paper/test_paper_broker_replace,alert/test_log_alert}/*`, `contracts/test_btest_imports.py`. **M2-IG.2:** `unit/adapters/shared/test_rate_limiter.py` (19 tests including G3-IB + G3-IG throttle tests under SimClock); `unit/adapters/shared/test_credentials.py` (16); `unit/domain/test_types.py` +3 for `Instrument.tradability`; `unit/runtime/test_broker_registry.py` (11). **M2-IG.3:** `unit/adapters/ig/test_credentials.py` (9); `unit/adapters/ig/test_client.py` (18 — URL routing, auth flow, refresh+reauth recovery, error classification, all under httpx.MockTransport); `unit/adapters/ig/test_instrument_resolver.py` (19 — epic-guess pure-function table, resolve happy path / cache, search fallback, zero/many candidates raise, family filtering, market_metadata + precision_for, malformed-response IGRequestInvalid). |
