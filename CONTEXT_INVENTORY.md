...
| 5. Code | `src/blive/` | DRAFT v1.4 (2026-05-01) | continuous | M0+M1: domain + adapters/{paper, memory, clock, alert} + strategy + sizing + risk + runtime/paper_pipeline. **M2-IG.2:** adapters/shared/{rate_limiter, credentials} + runtime/broker_registry + DD-1 v0.2 + DD-3 v0.2 substrate. **M2-IG.3:** adapters/ig/{credentials, client, instrument_resolver, broker, market_data, lightstreamer} + __init__.py factories. IGMarketData ships `historical_bars` (REST) + `subscribe_bars` (LightstreamerSource abstraction; production wrapper deferred). **M2-IG.4 (MARKET submit only):** IGBroker.submit() implements POST /positions/otc + GET /confirms/{ref} polling + FSM event emission (SUBMITTED → ACCEPTED+FILLED or REJECTED). dealReference→client_order_id mapping persisted for M5 reconciliation. cancel() / replace() raise NotImplementedError pointing at "working-order Phase 2 needs"; LIMIT/STOP submit also raises (MARKET-only path sufficient for ADR-039 Phase 1 strategy). **M2-IB.2:** adapters/ib/{credentials, client, rate_limiter, __init__}. IBClient wraps `ib_async.IB` (TCP socket + callback model); IBCredentials reads `~/.blive/secrets/ib.env` per ADR-035 (host/port/clientId/account_id only — no password); IB_DEFAULT_RATE_LIMITS table per ADR-031 + ADR-038 (`global` 20/s, `historical` 50/600s). Typed exceptions: IBError + IBConnectionError. **M2-IB.3a:** adapters/ib/instrument_resolver.py — IBInstrumentResolver per DD-7 §5 (refined v0.2). `to_contract` (sync; field-by-field Contract builder) + lazy `resolve` (qualifyContractsAsync + cache by Instrument tuple equality) + `clear_cache`. Typed exceptions: InstrumentNotResolvable + InstrumentAmbiguous. **scripts/probe_ib_handshake.py + probe_ib_resolve_contract.py** as wire-level smoke tests for the operator. |
| 6. Tests | `tests/` | DRAFT v1.4 (2026-05-01) | continuous | 411 tests green (was 393 at M2-IB.2). M1 + M2-IG.2 + M2-IG.3 + M2-IG.4 + M2-IB.2 modules unchanged. **M2-IB.3a additions:** `unit/adapters/ib/test_instrument_resolver.py` adds 18 tests covering `to_contract` field mapping (DD-7 §1/§2/§3), tradability rejection (CFD / spread_bet), unknown asset_class/venue rejection, `resolve` happy path + cache + rate-limit acquire, zero / multiple / zero-conId candidate paths, `clear_cache` per-instrument and full-flush, no-cache-on-failure, defensive None / nested-list filtering of ib_async return shape. Same `MagicMock(spec=ib_async.IB)` + `AsyncMock` injection pattern. |
...
| **KB-10** | [`docs/decisions/DECISIONS.md`](./docs/decisions/DECISIONS.md) | DRAFT v0.11 (2026-04-28) | ADR-001..031, 033..040 **ACCEPTED**. ADR-032 stays PROPOSED (IB-specific instrument resolution; revisit when M2-IB.3 IBInstrumentResolver exercises `qualifyContractsAsync` against IB Paper). Operator-approval moment 2026-04-27: eight ADRs flipped en bloc (ADR-030 per-archetype btest dispatch resolves OQ-030; ADR-033 AccountUpdate cadence; ADR-034 multi-broker registry; ADR-035 secrets handling; ADR-036 IG driver; ADR-037 Instrument.tradability field; ADR-038 IG rate-limit defaults; ADR-039 Phase 1 under IG bridge). M2-IB.2 (2026-04-28): ADR-031 token-bucket rate limiter shape ACCEPTED. M2-IB.3-prereq (2026-04-28): ADR-040 Phase 1 deployment target ACCEPTED (Windows native IB Gateway; Docker / IBC / Linux VM deferred to M8 production cutover). | Claude record, Oleg approve | continuous |
...
| **DD-7** | [`docs/dd/instrument_dictionary.md`](./docs/dd/instrument_dictionary.md) | DRAFT v0.2 (2026-05-01) | `Instrument` field-by-field map to IB `Contract`; `AssetClass` → `secType` table; `venue` (MIC) → IB `exchange` table (Phase 1: `XPAR → SBF`); ConID lazy lookup + in-memory cache; ambiguity discipline. Substrate for ADR-032; STABLE flip when M2-IB.3a IBInstrumentResolver exercises `qualifyContractsAsync` against IB Paper. **v0.2 (M2-IB.3a)**: §5 public surface refined to `IBInstrumentResolver(client: IBClient)` (was `(ib, rate_limiter)` sketch) — matches IG analogue + simplifies factory wiring. Functional contract unchanged. | DESIGN |
...
13. **M2-IB resumption (current active path)** — sub-milestones M2-IB.1 (substrate at `M2-substrate-IB.checkpoint`) / .2 (IBClient + IBCredentials) / .3 (IBInstrumentResolver + IBBroker read + IBMarketData) / .4 (write side + reconciliation, optionally consolidated with M3) / .5 (strategy run on IB Paper for ≥ 5 trading days + RETRO-M2-IB). Mirror M2-IG file structure 1:1 per [RETRO-M2-IG §"Recommendations"](./docs/retros/M2-IG_retrospective.md#recommendations-for-next_promptmd-v04-m2-ib-resumption). Reuse `blive.adapters.shared.{rate_limiter, credentials}` + `runtime.broker_registry` unchanged. ADR-031 + ADR-032 flip PROPOSED → ACCEPTED on first IB exercise; DD-7 / KB-2 / KB-3 STABLE flip on first IB Paper handshake. See [`NEXT_PROMPT.md`](./NEXT_PROMPT.md) v0.4 for the kickoff prompt.
    ...
    - **M2-IB.3a — IBInstrumentResolver (architectural surface).** Tag `M2-IB.3a-resolver` 2026-05-01. `blive.adapters.ib.instrument_resolver.IBInstrumentResolver` ships per DD-7 §5 v0.2 (refined to `(client: IBClient)` constructor); 18 unit tests against `MagicMock(spec=ib_async.IB)`. `scripts/probe_ib_resolve_contract.py` written for operator wire-level test. **Operator action OPEN**: run probe, paste output. ADR-032 + DD-7 STABLE flips deferred to the next commit (`M2-IB.3a-resolved`) on probe success.
    - **M2-IB.3b — IBBroker read + IBMarketData.** Blocked on M2-IB.3a probe success. Will surface ADR-033 AccountUpdate event implementation, KB-2 / KB-3 STABLE-flip exercise of §1-§9 surfaces.
