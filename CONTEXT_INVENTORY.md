...
| 5. Code | `src/blive/` | DRAFT v1.1 (2026-04-28) | continuous | M0+M1: domain + adapters/{paper, memory, clock, alert} + strategy + sizing + risk + runtime/paper_pipeline. **M2-IG.2:** adapters/shared/{rate_limiter, credentials} + runtime/broker_registry + DD-1 v0.2 + DD-3 v0.2 substrate. **M2-IG.3 (read-side complete + streaming via abstraction):** adapters/ig/{credentials, client, instrument_resolver, broker, market_data, lightstreamer} + __init__.py factories `create_ig_broker` / `create_ig_market_data`; both wired into `runtime/broker_registry`. IGMarketData ships `historical_bars` over REST AND `subscribe_bars` over a `LightstreamerSource` Protocol abstraction (concurrent-subscription budget per KB-17 §3 via asyncio.Semaphore). Production Lightstreamer wrapper (around `lightstreamer-client-lib`) is a deferred follow-up; until then `subscribe_bars` requires an explicit `lightstreamer_source` kwarg or raises NotImplementedError. IGBroker submit/cancel/replace stubs pending M2-IG.4. |
| 6. Tests | `tests/` | DRAFT v1.1 (2026-04-28) | continuous | 355 tests green. M1 + M2-IG.2 + earlier M2-IG.3 modules unchanged. **M2-IG.3 streaming additions:** `unit/adapters/ig/test_lightstreamer.py` (12 — Protocol conformance, FakeLightstreamerSource lifecycle, push/iterate, multi-subscription independence, unsubscribe + disconnect close streams, push-after-close raises, missing item raises, deterministic block-until-push); `unit/adapters/ig/test_market_data.py` grew to 44 tests (+17: Lightstreamer resolution + chart-item helpers, _stream_ohlc + _build_bar_from_state pure tests, subscribe_bars yields-on-CONS_END / fields-set / multi-bar / skip-malformed-consolidation / double-subscribe-raises / no-source-raises, unsubscribe releases budget + idempotent unknown, concurrent budget blocks, subscribe-failure releases budget). |
