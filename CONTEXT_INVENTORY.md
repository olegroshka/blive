...
>
> **Status:** v0.11 DRAFT — **M2-IB.6 closed 2026-05-06.** The Wed 2026-05-06 LSE-RTH wire run produced the first IB-paper FILL on M2-IB.6 (IBTM, 19 × £128.5) via the ADR-048 SMART/LSEETF routing. ADR-048 (LSE-ETF SMART discriminator) and ADR-049 (`OrderType.ADAPTIVE_MKT` + empirical PMA-cap finding) flipped PROPOSED → ACCEPTED at close. DD-7 §3 amended (XLON split by `asset_class`). [INV-14 v0.7](./docs/inv/ib_error_codes.md) catalogues IB warning 2161 (Price Management Algo regulatory cap on UK retail leveraged ETPs) with the four-run validation matrix. [OQ-031](./docs/decisions/OPEN_QUESTIONS.md#oq-031--phase-1-deployment-under-pma-bound-retail-account) raised for Phase 1 deployment decision; operator decided to address in **M3**, not block M2-IB.6 close. RETRO-M2-IB written + frozen. Next active path: **Phase 2 readiness audit** per [CONTEXT_PROTOCOL §8.3.2](./CONTEXT_PROTOCOL.md) — separate session from this close (kickoff prompt at NEXT_PROMPT v0.8).
>
...
| 0. Index | **this file** (`CONTEXT_INVENTORY.md`) | DRAFT v0.10 (2026-05-06) | continuous | the registry of all artifacts |
