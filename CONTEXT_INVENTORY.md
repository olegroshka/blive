...
>
> **Status:** v0.13 DRAFT — **M3 plan-drafted 2026-05-06; M2 → Phase 2 transition complete (§8.3.2 three-session pattern fully discharged).** [`TASK_REGISTRY.md`](./TASK_REGISTRY.md) v0.5 → v0.6: legacy "M3 — IB Adapter (Write Side)" section replaced with the deployment-decision M3 plan (sub-milestones M3.1 unit-of-quote reconciliation → M3.2 10-day empirical window → M3.3 OQ-031 resolution → M3.4 mixed-currency P&L → M3.5 INV-14 + chaos drills → M3.6 KB-2/3 STABLE → M3-close). Five plan-drafting calls recorded inline (Q1 inform-then-resolve / Q2 10-day calendar-bound window / Q3 EODHD-vs-IB pull forward to M3 / Q4 A3-only / Q5 stub-DRAFT only what M3 produces). G4 gate rewritten around the 10 deployment-decision exit criteria. Sketched M4+ refreshed (M4 picks up RC-01..RC-07 + RC-11; M5 loses Phase 2 readiness audit; M5/M7 stub-DRAFT framings shift to "extends M3's stub"). Risk register grows six Phase-2-entry rows. No new ADRs / OQs raised — substrate-only per [CONTEXT_PROTOCOL §8.3.2](./CONTEXT_PROTOCOL.md). Per the protocol the M2 → Phase 2 transition required (1) implementation close ✓ at `M2-IB.6-close` (RETRO-M2-IB frozen); (2) readiness audit ✓ at commit `c5fb1b4` (PHASE_2_READINESS.md DRAFT v0.1); (3) **M3 plan-drafting ✓ this session** (TASK_REGISTRY v0.6 + NEXT_PROMPT v1.0). Next active path: **M3.1 — EODHD-vs-IB unit-of-quote reconciliation** (kickoff prompt at NEXT_PROMPT v1.0). Prior banner content preserved in [PHASE_2_READINESS.md](./docs/PHASE_2_READINESS.md), [RETRO-M2-IB](./docs/retros/M2-IB_retrospective.md), and §10 history below.
>
...
| 0. Index | **this file** (`CONTEXT_INVENTORY.md`) | DRAFT v0.13 (2026-05-06) | continuous | the registry of all artifacts |
...
16. **M3.1 — EODHD-vs-IB unit-of-quote reconciliation.** Investigate QQL3's ~10× EODHD-vs-IB price discrepancy ([RETRO-M2-IB](./docs/retros/M2-IB_retrospective.md) §"Surprises" #7); narrow-scope sizing fix (Route A IB live market data vs Route B EODHD-convention conversion — operator-decision); implement RC-10 (price sanity) per [INV-4](./docs/inv/risk_checks.md); KB-15 stub-DRAFT (unit-of-quote section). Kickoff prompt at [`NEXT_PROMPT.md`](./NEXT_PROMPT.md) v1.0. **Front of the queue.**
