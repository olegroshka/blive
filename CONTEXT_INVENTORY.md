...
| 5. Code | `src/blive/` | DRAFT v1.0 (2026-04-28) | continuous | M0+M1: `domain/{types,events,order_fsm,ports,positions}.py`; `adapters/{paper.{broker,market_data}, memory.{persistence,bus}, clock.{wall,sim}, alert.log}`; `strategy/{config,loader}.py`; `sizing/sizer.py`; `risk/checks.py`; `runtime/paper_pipeline.py`. **M2-IG.2:** `adapters/shared/{rate_limiter,credentials}.py`; `runtime/broker_registry.py`; `domain/types.py` v0.2; `strategy/config.py` v0.2. **M2-IG.3 (read-side complete; Lightstreamer streaming pending):** `adapters/ig/{credentials,client,instrument_resolver,broker,market_data}.py` + `__init__.py` factories `create_ig_broker` / `create_ig_market_data`; both wired into `runtime/broker_registry`. IGMarketData ships `historical_bars` over REST; `subscribe_bars` / `subscribe_trades` raise NotImplementedError pending M2-IG.3 follow-up. IGBroker submit/cancel/replace stubs pending M2-IG.4. |
| 6. Tests | `tests/` | DRAFT v1.0 (2026-04-28) | continuous | 326 tests green. M1 added `unit/{strategy,sizing,risk,runtime}/*`, `unit/adapters/{paper/test_market_data,paper/test_paper_broker_replace,alert/test_log_alert}/*`, `contracts/test_btest_imports.py`. **M2-IG.2:** `unit/adapters/shared/test_rate_limiter.py` (19); `unit/adapters/shared/test_credentials.py` (16); `unit/domain/test_types.py` +3 for `Instrument.tradability`; `unit/runtime/test_broker_registry.py` (12 incl. IG bootstrap). **M2-IG.3:** `unit/adapters/ig/test_credentials.py` (9); `unit/adapters/ig/test_client.py` (18); `unit/adapters/ig/test_instrument_resolver.py` (19); `unit/adapters/ig/test_broker.py` (27); `unit/adapters/ig/test_market_data.py` (27 — freq/duration mapping tables, _format_ig_datetime UTC + naive-rejects, _ohlc_value lastTraded/mid fallback, _parse_ig_snapshot ISO + slash, _parse_price_bar full + mid + negative-volume + missing-snapshot, historical_bars parsed/empty/inverted-range/naive-raises/skip-malformed/historical_prices-bucket-blocking, subscribe_bars + subscribe_trades NotImplementedError, unsubscribe noop). All under httpx.MockTransport. |
