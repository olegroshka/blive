...
| 5. Code | `src/blive/` | DRAFT v1.2 (2026-04-28) | continuous | M0+M1: domain + adapters/{paper, memory, clock, alert} + strategy + sizing + risk + runtime/paper_pipeline. **M2-IG.2:** adapters/shared/{rate_limiter, credentials} + runtime/broker_registry + DD-1 v0.2 + DD-3 v0.2 substrate. **M2-IG.3:** adapters/ig/{credentials, client, instrument_resolver, broker, market_data, lightstreamer} + __init__.py factories. IGMarketData ships `historical_bars` (REST) + `subscribe_bars` (LightstreamerSource abstraction; production wrapper deferred). **M2-IG.4 (MARKET submit only):** IGBroker.submit() implements POST /positions/otc + GET /confirms/{ref} polling + FSM event emission (SUBMITTED → ACCEPTED+FILLED or REJECTED). dealReference→client_order_id mapping persisted for M5 reconciliation. cancel() / replace() raise NotImplementedError pointing at "working-order Phase 2 needs"; LIMIT/STOP submit also raises (MARKET-only path sufficient for ADR-039 Phase 1 strategy). |
| 6. Tests | `tests/` | DRAFT v1.2 (2026-04-28) | continuous | 359 tests green. M1 + M2-IG.2 + M2-IG.3 modules unchanged. **M2-IG.4 additions:** `unit/adapters/ig/test_broker.py` grew to 31 tests (+4 net: replaced single-stub test_submit_raises with 5 substantive tests — submit_market_order_happy_path verifying SUBMITTED+ACCEPTED+FILLED event sequence + Fill payload, submit_market_order_rejected for INSUFFICIENT_FUNDS path, submit_polls_until_confirm_resolves for the OPEN→ACCEPTED transition over 3 polls, non-MKT order raises NotImplementedError, before_connect raises RuntimeError; cancel/replace tests updated to match new "working orders" message). |
