...
| 5. Code | `src/blive/` | DRAFT v0.6 (2026-04-28) | continuous | M0+M1: `domain/{types,events,order_fsm,ports,positions}.py`; `adapters/{paper.{broker,market_data}, memory.{persistence,bus}, clock.{wall,sim}, alert.log}`; `strategy/{config,loader}.py`; `sizing/sizer.py`; `risk/checks.py`; `runtime/paper_pipeline.py`. **M2-IG.2:** `adapters/shared/{rate_limiter,credentials}.py` (broker-agnostic token bucket per ADR-031+ADR-038; env-var/dotenv credentials loader per ADR-035 with redaction-list helper); `runtime/broker_registry.py` (multi-broker dispatch per ADR-034); `domain/types.py` v0.2 (Tradability + Instrument.tradability per ADR-037); `strategy/config.py` v0.2 (LiveStrategyConfig.broker per ADR-034). **M2-IG.3 in flight:** `adapters/ig/{__init__,credentials}.py` (IG_SCHEMA per ADR-035 + KB-17 §4; typed IGCredentials wrapper validating IG_ENVIRONMENT). |
| 6. Tests | `tests/` | DRAFT v0.6 (2026-04-28) | continuous | 233 tests green. M1 added `unit/{strategy,sizing,risk,runtime}/*`, `unit/adapters/{paper/test_market_data,paper/test_paper_broker_replace,alert/test_log_alert}/*`, `contracts/test_btest_imports.py`. **M2-IG.2:** `unit/adapters/shared/test_rate_limiter.py` (19 tests including G3-IB + G3-IG throttle tests under SimClock); `unit/adapters/shared/test_credentials.py` (16 tests); `unit/domain/test_types.py` +3 tests for `Instrument.tradability`; `unit/runtime/test_broker_registry.py` (11 tests). **M2-IG.3:** `unit/adapters/ig/test_credentials.py` (9 tests covering IG_SCHEMA shape, secret-field discrimination, typed-load flow, IG_ENVIRONMENT validation). |
