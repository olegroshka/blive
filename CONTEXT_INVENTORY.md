...
>
> **Status:** v0.9 DRAFT — Phase 1 strategy switch per [ADR-043](./docs/decisions/DECISIONS.md#adr-043--phase-1-strategy-switch-triple_lev_sma_filter_dsl-a3-replaces-tkan_v4_momentum_timing-a2) (2026-05-02): A3 (`triple_lev_sma_filter_dsl` on TQQQ/TMF/IEF) replaces A2 (`tkan_v4_momentum_timing` on CAC.PA); M2-IB.5 closed at architectural surface (CLOSED-EARLY-BY-OPERATOR); M2-IB.6 milestone opens with sub-milestones .6-substrate / .6.1 / .6.2 / .6-close; ADR-021 SUPERSEDED-BY-ADR-043; companion ADRs 044/045/046 follow in the next commit (multi-instrument pipeline, LongShortPortfolio btest dispatch, IB SMART for US equities). Prior v0.8 baseline: methodology-amendment batch added Layer 0. Bootstrap (`CLAUDE.md`) per [ADR-042](./docs/decisions/DECISIONS.md#adr-042--session-bootstrap-files-agent-agnostic-pattern-for-l0-warm-up-entry-point); CONTEXT_PROTOCOL §11.2 extended; Amendments_Log v0.3 recorded.
>
...
| 5. Code | `src/blive/` | DRAFT v1.6 (2026-05-01) | continuous | M0+M1: domain + adapters/{paper, memory, clock, alert} + strategy + sizing + risk + runtime/paper_pipeline. **M2-IG:** adapters/shared/{rate_limiter, credentials} + runtime/broker_registry + adapters/ig/* + DD-1 v0.2 / DD-3 v0.2 substrate. **M2-IB.2:** adapters/ib/{credentials, client, rate_limiter, __init__}. IBClient wraps `ib_async.IB` (TCP + callback). **M2-IB.3a:** adapters/ib/instrument_resolver.py — IBInstrumentResolver + Yahoo-suffix translation per ADR-041. **M2-IB.3b-i + .3b-i-timer:** adapters/ib/broker.py — IBBroker (read methods + 30s AccountUpdate diff-suppress timer per ADR-033). domain/events.py + ports.py widened with AccountUpdate + ArtefactFreshnessWarning. **M2-IB.3b-ii:** adapters/ib/market_data.py — IBMarketData (historical_bars via reqHistoricalDataAsync; subscribe_* raise NotImplementedError pending M2-IB.5). create_ib_broker + create_ib_market_data factories registered in broker_registry. **scripts/probe_ib_*.py** = 4 wire-level smoke tests (handshake / resolve_contract / read / market_data). |
| 6. Tests | `tests/` | DRAFT v1.6 (2026-05-01) | continuous | 465 tests green (was 449 at M2-IB.3b-i-timer). M0/M1/M2-IG/M2-IB.2/.3a/.3b-i modules unchanged. **M2-IB.3b-ii additions:** `unit/adapters/ib/test_market_data.py` adds 16 tests for IBMarketData — historical_bars happy path (chronological ordering, [start,end) filtering, contract translation including Yahoo-suffix stripping, BarFreq → IB barSize mapping); rate-limit acquire on `historical` bucket; BID_ASK doubles tokens; rejection paths (naive datetime / inverted range / unsupported freq / wire error wrapped in IBMarketDataError / unparseable bar skipped); volume=-1 clamped to 0; vwap=-1 / 0 → None; subscribe_bars / subscribe_trades raise NotImplementedError; unsubscribe is no-op. |
...
| **KB-10** | [`docs/decisions/DECISIONS.md`](./docs/decisions/DECISIONS.md) | DRAFT v0.14 (2026-05-02) | **ADR-001..043 ACCEPTED**, except **ADR-021 SUPERSEDED-BY-ADR-043** (Phase 1 strategy switch). v0.13 (methodology batch): ADR-042 (session-bootstrap files) ACCEPTED. v0.14 (Phase 1 strategy switch): ADR-043 (A3 `triple_lev_sma_filter_dsl` on TQQQ/TMF/IEF replaces A2 `tkan_v4_momentum_timing` on CAC.PA) ACCEPTED; ADR-021 SUPERSEDED-BY-ADR-043 (substrate durable — DD-7 §3 / §3.1, ADR-041, M2-IB.4a-happy-cacpa wire validation all stay; only the Phase 1 strategy designation moves). Companion ADRs 044/045/046 (multi-instrument pipeline, LongShortPortfolio btest dispatch, IB SMART for US equities) follow in the next commit. | Claude record, Oleg approve | continuous |
...
| **INV-14** | [`docs/inv/ib_error_codes.md`](./docs/inv/ib_error_codes.md) | DRAFT v0.2 (2026-05-02) | IB error codes blive's adapter handles (typed-exception map + operator action). M2-IB.3b-ii close: 162 (no SBF historical permissions) + 200 (security definition not found) observed. M2-IB.4a wire probe (2026-05-02): 10311 (direct-routing precaution → REJECTED via disambiguation), 201 (order rejected — promoted from forward-list), 10147 (cancel-after-terminal — diagnostic) added. Forward-list of KB-3 §8 codes (100/202/322/354/366/1100..1102/1300) remaining for M2-IB.4b / M5 promotion. | DESIGN |
...
13. **M2-IB resumption (current active path)** — sub-milestones M2-IB.1 (substrate at `M2-substrate-IB.checkpoint`) / .2 (IBClient + IBCredentials) / .3 (IBInstrumentResolver + IBBroker read + IBMarketData) / .4 (write side + reconciliation, optionally consolidated with M3) / .5 (strategy run on IB Paper for ≥ 5 trading days + RETRO-M2-IB). Mirror M2-IG file structure 1:1 per [RETRO-M2-IG §"Recommendations"](./docs/retros/M2-IG_retrospective.md#recommendations-for-next_promptmd-v04-m2-ib-resumption). Reuse `blive.adapters.shared.{rate_limiter, credentials}` + `runtime.broker_registry` unchanged. ADR-031 + ADR-032 flip PROPOSED → ACCEPTED on first IB exercise; DD-7 / KB-2 / KB-3 STABLE flip on first IB Paper handshake. See [`NEXT_PROMPT.md`](./NEXT_PROMPT.md) v0.4 for the kickoff prompt.
    ...
    - **M2-IB.6.1 — multi-instrument pipeline + 5-ticker EODHD refresh + LongShortPortfolio btest dispatch + IB SMART resolver convention.** PENDING. Companion ADRs 044/045/046 land in the next commit; pipeline / refresh-script / resolver code lands subsequently. Tests against synthetic 3-instrument fixtures.
    - **M2-IB.6.2 — IB Paper end-to-end run during US RTH (`triple_lev_sma_filter_dsl`).** PENDING. Driver `scripts/run_m2ib6_ib_paper.py`. Run during US session (14:30–21:00 UTC). FILLED + multi-instrument FSM coverage. INV-14 grows with any US-side codes observed.
    - **M2-IB.6-close — RETRO-M2-IB + NEXT_PROMPT.md v0.6.** PENDING. Frozen retro covering M2-IB.1 → M2-IB.6. NEXT_PROMPT v0.6 → Phase 2 readiness audit per [CONTEXT_PROTOCOL §8.3.2](./CONTEXT_PROTOCOL.md) phase-boundary protocol.
