...
| 5. Code | `src/blive/` | DRAFT v0.9 (2026-04-28) | continuous | M0+M1: `domain/{types,events,order_fsm,ports,positions}.py`; `adapters/{paper.{broker,market_data}, memory.{persistence,bus}, clock.{wall,sim}, alert.log}`; `strategy/{config,loader}.py`; `sizing/sizer.py`; `risk/checks.py`; `runtime/paper_pipeline.py`. **M2-IG.2:** `adapters/shared/{rate_limiter,credentials}.py`; `runtime/broker_registry.py`; `domain/types.py` v0.2; `strategy/config.py` v0.2. **M2-IG.3 in flight:** `adapters/ig/{__init__,credentials}.py`; `adapters/ig/client.py`; `adapters/ig/instrument_resolver.py`; `adapters/ig/broker.py` (IGBroker BrokerPort impl — read-side only: connect+PUT-/session account switch / disconnect / positions / account_snapshot / open_orders / events; IG-response parsers for /positions, /accounts, /workingorders; epic→AssetClass mapping; IG TIF/order-type→blive mapping; IG timestamp parsing both formats; submit/cancel/replace stubs raise NotImplementedError pending M2-IG.4). |
| 6. Tests | `tests/` | DRAFT v0.9 (2026-04-28) | continuous | 297 tests green. M1 added `unit/{strategy,sizing,risk,runtime}/*`, `unit/adapters/{paper/test_market_data,paper/test_paper_broker_replace,alert/test_log_alert}/*`, `contracts/test_btest_imports.py`. **M2-IG.2:** `unit/adapters/shared/test_rate_limiter.py` (19); `unit/adapters/shared/test_credentials.py` (16); `unit/domain/test_types.py` +3 for `Instrument.tradability`; `unit/runtime/test_broker_registry.py` (11). **M2-IG.3:** `unit/adapters/ig/test_credentials.py` (9); `unit/adapters/ig/test_client.py` (18); `unit/adapters/ig/test_instrument_resolver.py` (19); `unit/adapters/ig/test_broker.py` (27 — connect/PUT-session/disconnect; positions parser happy path + short direction + skip-malformed + empty + before-connect; account_snapshot active-account filter + not-found + empty; open_orders parser; events ConnectionStatus emission; submit/cancel/replace NotImplementedError stubs; helper-function tests for epic→symbol/asset_class, order-type/TIF mapping, IG timestamp parsing both formats, hex_like). All under httpx.MockTransport. |
