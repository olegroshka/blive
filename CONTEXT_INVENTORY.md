...
>
> **Status:** v0.12 DRAFT — **Phase 2 readiness audit complete 2026-05-06.** [`PHASE_2_READINESS.md`](./docs/PHASE_2_READINESS.md) DRAFT v0.1 written: eight-dimension audit informed by M2-IB.6's real wire outcomes; five cross-cutting questions raised as the agenda for the M3 plan-drafting session. No new ADRs raised; no new OQs raised (OQ-031 already covers the PMA-cap deployment question). Per [CONTEXT_PROTOCOL §8.3.2](./CONTEXT_PROTOCOL.md), the M2 → Phase 2 transition is the three-session pattern: (1) implementation close ✓ at `M2-IB.6-close` (RETRO-M2-IB frozen); (2) readiness audit ✓ this session (PHASE_2_READINESS.md DRAFT v0.1); (3) **M3 plan-drafting session** = next active path (kickoff prompt at NEXT_PROMPT v0.9). Prior banner content (M2-IB.6 close artefact ladder, first IB-paper FILL on IBTM 19 × £128.5, ADR-048 + ADR-049 ACCEPTED, DD-7 §3 amended, INV-14 v0.7 PMA-cap matrix, OQ-031 deferred-to-M3) is preserved in [RETRO-M2-IB](./docs/retros/M2-IB_retrospective.md) and §10 history below.
>
...
| 0. Index | **this file** (`CONTEXT_INVENTORY.md`) | DRAFT v0.12 (2026-05-06) | continuous | the registry of all artifacts |
...
| 4. Plan | [`TASK_REGISTRY.md`](./TASK_REGISTRY.md) | DRAFT v0.5 (2026-05-06) | per milestone | Phase 1 plan: M0–M1 closed; M2-IG bridge **closed at architectural surface 2026-04-28** (sub-milestones .1–.4 shipped; .5 strategy run deferred — see [RETRO-M2-IG](./docs/retros/M2-IG_retrospective.md)); **M2-IB active** through M2-IB.6. M2-IB.1–.5 are complete at the substrate / architectural-surface level; **M2-IB.6.1 is complete** (multi-instrument pipeline + LongShortPortfolio dispatch + PRIIPs-compliant universe substitution + LSEETF resolver fix), **M2-IB.6.2 is partial** (US-RTH PRIIPs probe complete; next available LSE-RTH filled run still pending), and **M2-IB.6-close remains open** (RETRO-M2-IB + successor `NEXT_PROMPT.md`). |
| 5. Code | `src/blive/` | DRAFT v1.7 (2026-05-04) | continuous | M0+M1: domain + adapters/{paper, memory, clock, alert} + strategy + sizing + risk + runtime/paper_pipeline. **M2-IG:** adapters/shared/{rate_limiter, credentials} + runtime/broker_registry + adapters/ig/*. **M2-IB.2–.4a:** adapters/ib/{credentials, client, instrument_resolver, broker, market_data, rate_limiter}; IB read/write side wire-validated. **M2-IB.5:** `runtime/ib_pipeline.py`, `runtime/signals.py`, `scripts/refresh_eodhd_parquet.py`, `scripts/run_m2ib5_paper.py`. **M2-IB.6.1 / .6.2-pre:** multi-instrument IB pipeline + LongShortPortfolio dispatch + `scripts/refresh_eodhd_signals.py` + `scripts/run_m2ib6_ib_paper.py` + `scripts/probe_tqqq_us_rth.py`; resolver now handles UK-listed ETFs via `SMART` + `primaryExchange="LSEETF"`; broker now subscribes to `ib.errorEvent` and stashes order-linked errors so PRIIPs / `Inactive` rejections surface the real formatted reason text instead of the fallback literal. |
| 6. Tests | `tests/` | DRAFT v1.7 (2026-05-04) | continuous | 517 unit tests green at head `abebc5a`. Earlier M0/M1/M2-IG/M2-IB.2/.3/.4 coverage retained. **M2-IB.6.1 additions:** multi-instrument pipeline / LongShortPortfolio dispatch / resolver-routing tests. **M2-IB.6.2 additions:** broker tests for the `statusEvent`-before-`errorEvent` PRIIPs rejection path and for ignoring connection-level `reqId <= 0` events in the order-error stash. |
...
| **KB-10** | [`docs/decisions/DECISIONS.md`](./docs/decisions/DECISIONS.md) | DRAFT v0.17 (2026-05-03) | **ADR-001..047 ACCEPTED**, except **ADR-021 SUPERSEDED-BY-ADR-043**. v0.15 added ADR-044/045/046 (multi-instrument pipeline, LongShortPortfolio dispatch, IB SMART for US equities). v0.16 added ADR-047 (PRIIPs-compliant UK-listed Phase 1 universe). v0.17 adds **ADR-048 PROPOSED** in the working tree (LSE-ETF SMART routing discriminator; held until LSE-RTH filled validation). | Claude record, Oleg approve | continuous |
...
| **INV-14** | [`docs/inv/ib_error_codes.md`](./docs/inv/ib_error_codes.md) | DRAFT v0.6 (2026-05-04) | IB error codes blive's adapter handles (typed-exception map + operator action). M2-IB.3b-ii: 162 + 200 observed. M2-IB.4a: 10311 / 201 / 10147 / 202 / 399 promoted. **M2-IB.6.1 / .6.2:** PRIIPs-KID variant of 201 documented, plus the broker-side **reason-extraction taxonomy**: `trade.log` alone is insufficient for `Inactive` rejections; `ib.errorEvent` stash + deferred emission are now load-bearing. Forward-list of KB-3 §8 codes (100 / 322 / 354 / 366 / 1100..1102 / 1300) remains for later promotion. | DESIGN |
...
**Latest reconciliation (2026-05-06 / Phase 2 readiness audit complete):** [`PHASE_2_READINESS.md`](./docs/PHASE_2_READINESS.md) DRAFT v0.1 written this session as the second of the three [CONTEXT_PROTOCOL §8.3.2](./CONTEXT_PROTOCOL.md) phase-boundary sessions. Eight-dimension audit informed by M2-IB.6's real wire outcomes (PRIIPs / KID hard block, LSE-ETF SMART discriminator, structural PMA-cap on UK retail leveraged ETPs). Five cross-cutting questions raised as the agenda for the M3 plan-drafting session: (1) OQ-031 sequencing; (2) empirical fill-rate window scope; (3) EODHD-vs-IB unit-of-quote reconciliation timing; (4) strategy-slot scope (A3-only vs A1/A1a parallel); (5) Phase 2 substrate prerequisites. No new ADRs raised; no new OQs raised. Next active path: **M3 plan-drafting session** per CONTEXT_PROTOCOL §8.3.2 (kickoff prompt at NEXT_PROMPT v0.9).
...
15. **M3 plan-drafting session** (per [CONTEXT_PROTOCOL §8.3.2](./CONTEXT_PROTOCOL.md) phase-boundary protocol — third and final session of the M2 → Phase 2 transition). Operator-led on the five cross-cutting questions in [`docs/PHASE_2_READINESS.md`](./docs/PHASE_2_READINESS.md); agent drafts the M3 plan in `TASK_REGISTRY.md` (or splits to a phase-specific `TASK_REGISTRY_PHASE_2.md` if scope warrants). Kickoff prompt at [`NEXT_PROMPT.md`](./NEXT_PROMPT.md) v0.9. **Front of the queue.**
