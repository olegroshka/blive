...
>
> **Status:** v0.6 DRAFT — M2-IG.1 substrate ACCEPTED en bloc (eight ADRs flipped PROPOSED → ACCEPTED; ADR-031, ADR-032 stay PROPOSED). Will be edited every time an artifact is added, retired, or moved.
>
...
| 4. Plan | [`TASK_REGISTRY.md`](./TASK_REGISTRY.md) | DRAFT v0.2 (2026-04-27) | per milestone | Phase 1 plan: M0–M1 closed; M2 split into M2-IB (PARKED at `M2-substrate-IB.checkpoint` pending IB account reopen) + M2-IG (ACTIVE bridge with sub-milestones .1–.5); M3 unchanged; M4+ sketched. Quality gates split G3 → G3-IB / G3-IG. Risk register and open-dependencies updated for IG bridge. |
| 5. Code | `src/blive/` | DRAFT v0.7 (2026-04-28) | continuous | M0+M1: `domain/{types,events,order_fsm,ports,positions}.py`; `adapters/{paper.{broker,market_data}, memory.{persistence,bus}, clock.{wall,sim}, alert.log}`; `strategy/{config,loader}.py`; `sizing/sizer.py`; `risk/checks.py`; `runtime/paper_pipeline.py`. **M2-IG.2:** `adapters/shared/{rate_limiter,credentials}.py`; `runtime/broker_registry.py` (multi-broker dispatch per ADR-034); `domain/types.py` v0.2 (Tradability + Instrument.tradability per ADR-037); `strategy/config.py` v0.2 (LiveStrategyConfig.broker per ADR-034). **M2-IG.3 in flight:** `adapters/ig/{__init__,credentials}.py` (IG_SCHEMA + typed IGCredentials per KB-17); `adapters/ig/client.py` (REST core per ADR-036 — IGClient with httpx.AsyncClient, 3-step auth, refresh+reauth recovery loop, typed exception hierarchy IGError/IGAuthError/IGSessionExpired/IGRateLimited/IGOrderRejected/IGRequestInvalid/IGConnectionError; KB-17 §6 errorCode → exception classifier). |
| 6. Tests | `tests/` | DRAFT v0.7 (2026-04-28) | continuous | 251 tests green. M1 added `unit/{strategy,sizing,risk,runtime}/*`, `unit/adapters/{paper/test_market_data,paper/test_paper_broker_replace,alert/test_log_alert}/*`, `contracts/test_btest_imports.py`. **M2-IG.2:** `unit/adapters/shared/test_rate_limiter.py` (19 tests, G3-IB + G3-IG throttle tests under SimClock); `unit/adapters/shared/test_credentials.py` (16 tests); `unit/domain/test_types.py` +3 tests for `Instrument.tradability`; `unit/runtime/test_broker_registry.py` (11 tests). **M2-IG.3:** `unit/adapters/ig/test_credentials.py` (9 tests); `unit/adapters/ig/test_client.py` (18 tests — URL routing demo/live, 3-step auth, idempotent connect, missing-headers raise, disconnect, header propagation, pre-connect-raises, trading-bucket-blocking, error mapping for rate-limit / invalid-instrument / deal-rejected, session-expired refresh-and-retry, failed-refresh full-reauth fallback, persistent auth failure, recoverable session-expired hidden from caller, classifier table). All under httpx.MockTransport. |
...
| **KB-10** | [`docs/decisions/DECISIONS.md`](./docs/decisions/DECISIONS.md) | DRAFT v0.9 (2026-04-27) | ADR-001..030, 033, 034..039 **ACCEPTED**. ADR-031, ADR-032 stay PROPOSED (IB-specific; revisit when M2-IB resumes). Operator-approval moment 2026-04-27: eight ADRs flipped en bloc (ADR-030 per-archetype btest dispatch resolves OQ-030; ADR-033 AccountUpdate cadence; ADR-034 multi-broker registry; ADR-035 secrets handling; ADR-036 IG driver; ADR-037 Instrument.tradability field; ADR-038 IG rate-limit defaults; ADR-039 Phase 1 under IG bridge). | Claude record, Oleg approve | continuous |
...
| **DD-2** | [`docs/dd/event_schemas.md`](./docs/dd/event_schemas.md) | DRAFT v0.1 (2026-04-27) | Field-level for `OrderEvent` / `ConnectionStatus` (M0; defer to DD-1), `RiskBreach` (M1), `AccountUpdate` (M2 per ADR-033), `ArtefactFreshnessWarning` (M2 per ADR-022). M4/M5/M7 events catalogued in INV-5 but not yet detailed here. | DESIGN |
...
| **DD-7** | [`docs/dd/instrument_dictionary.md`](./docs/dd/instrument_dictionary.md) | DRAFT v0.1 (2026-04-27) | `Instrument` field-by-field map to IB `Contract`; `AssetClass` → `secType` table; `venue` (MIC) → IB `exchange` table (Phase 1: `XPAR → SBF`); ConID lazy lookup + in-memory cache; ambiguity discipline. Substrate for ADR-032; STABLE flip when M2 IBBroker exercises the path against IB Paper. | DESIGN |
...
6. **M2-IB execution** — PARKED 2026-04-27 pending operator's IB Paper account reopening. Resumes when IB account ready and M2-IG bridge has stabilised. Per [TASK_REGISTRY M2-IB](./TASK_REGISTRY.md).
...
8. **M2-IG.1 batch 2 substrate (current commit, awaiting operator review)** — IG-specific ADRs: ADR-036 (IG wire-level driver: roll-our-own httpx + asyncio Lightstreamer), ADR-037 (`Instrument.tradability` field), ADR-038 (IG rate-limit defaults; parameterises ADR-031 with named-bucket config), ADR-039 (Phase 1 strategy under IG bridge: CAC 40 CFD; ADR-021 PAUSED). New substrate files: KB-16 (IG capability matrix), KB-17 (IG pacing spec), DD-8 (IG instrument dictionary). DD-3 / DD-1 / INV-6 / KB-8 amendments deferred to M2-IG.2 alongside their code counterparts.
9. **M2-IG.2 cross-cutting infra code (in progress)** — ✓ `blive.adapters.shared.rate_limiter` landed 2026-04-27 (broker-agnostic token bucket per [ADR-031](./docs/decisions/DECISIONS.md#adr-031--token-bucket-rate-limiter-shape-for-ib-adapters) + [ADR-038](./docs/decisions/DECISIONS.md#adr-038--ig-rate-limit-defaults-parameterise-adr-031); 19 unit tests including the G3-IB and G3-IG throttle tests at unit scale). Still to land: `blive.adapters.shared.credentials` ([ADR-035](./docs/decisions/DECISIONS.md#adr-035--secrets-handling-discipline-blivesecrets)), `blive.runtime.broker_registry` ([ADR-034](./docs/decisions/DECISIONS.md#adr-034--multi-broker-registry-pattern-extends-adr-004)), DD-1 amendment for `tradability` ([ADR-037](./docs/decisions/DECISIONS.md#adr-037--instrumenttradability-field-spot--cfd--spread_bet)), DD-3 amendment for top-level `broker` field, INV-6 amendments for IG adapter rows, new `import-linter` contract `Broker registry isolation (ADR-034)`, `secrets/ig.env.example` + `.gitignore` rule, KB-8 IG amendment.
10. **M2-IG.3, M2-IG.4, M2-IG.5** — IG read-side adapter (`IGClient`, `IGBroker` read methods, `IGMarketData`, `IGInstrumentResolver`), write-side adapter (`IGBroker.submit/cancel/replace`), strategy run + retro. See [TASK_REGISTRY M2-IG](./TASK_REGISTRY.md) sub-milestones.
