...
>
> **Status:** v0.25 DRAFT — **M3 CLOSED 2026-06-05 — G4 PASSED (all 10 exit criteria); Phase 1 (M0→M3) complete.** M3 closed across M3.1→M3.6: ADR-050/051 (unit-of-quote magnitude + tick-grid) → bounded M3.2 capture (QQL3 0/69 fills vs IBTM 6/6) → **ADR-052** (OQ-031 Option 1: accept the PMA-bound leveraged-leg non-fill) + **OQ-032** (the leverage trilemma — PRIIPs / PMA / Cash each block leveraged equity exposure) → **M3.4** (found + fixed the `AccountSnapshot.equity`-reads-base-sleeve bug; DD-1 v0.3) → **M3.5** (chaos drill → KB-7 FM-1 + IB 10141; native auto-reconnect = M5) → **M3.6** (KB-2 / KB-3 → STABLE v1.0). [RETRO-M3](./docs/retros/M3_retrospective.md) frozen. Tests 591; all gates green (formatters pinned `black==26.3.1` / `isort==8.0.1`). **Next: the [§8.3.2](./CONTEXT_PROTOCOL.md) M3→M4 / Phase-1→Phase-2 boundary — readiness-audit refresh ([NEXT_PROMPT](./NEXT_PROMPT.md) v1.5) then plan-drafting (OQ-032 + the VIX/TQQQ/VXX notebook).** **Banner trimmed at M3 close (§6.4 freeze):** the full per-increment M3.1→M3.6 banner history (~v0.14–v0.23) is preserved verbatim in [`docs/_freezes/M3-CONTEXT_INVENTORY.md`](./docs/_freezes/M3-CONTEXT_INVENTORY.md); earlier milestones live in the retros (RETRO-M0 / M1 / M2-IG / M2-IB / M3) and the §10 log below. **Post-M3 tooling (ADR-053, 2026-06-05):** blive + btest upgraded **Python 3.11 → 3.12** (for Phase 2's VIX strategy / OQ-032); `.venv` on 3.12.12, `pandas` pinned `<3`, `mypy==2.1.0`, btest `signum` dep removed — all gates green (591).
>
...
| 0. Index | **this file** (`CONTEXT_INVENTORY.md`) | DRAFT v0.25 (2026-06-05) | continuous | the registry of all artifacts |
