...
None blocking. STABLE flip when M2-IB.4 has populated the §"Error catalogue" with the order-side codes (202, 322 still on the forward list) — at that point the catalogue is no longer a stub against the §"Codes catalogued in KB-3 but not yet observed" forward list. The disambiguation path at code 10311 + 201 is now wire-validated (M2-IB.4a probe, 2026-05-02); the happy-path SUBMITTED → ACCEPTED → CANCELED still wants validation once the operator allows direct-routed orders via TWS API → Precautions.
