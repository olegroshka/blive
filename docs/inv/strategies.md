...
|-------------|------------|-----------|----------|------|-------|--------------|----------|-----------|
| `triple_lev_sma_filter_dsl` | `research/Triple Leveraged ETF/triple_leveraged_etf_dsl.ipynb` | A3 | **Phase 1 deployment universe (UK retail, PRIIPs-compliant per [ADR-047](../decisions/DECISIONS.md#adr-047--priips-compliant-universe-for-phase-1-a3-strategy-refines-adr-043))**: `QQL3` (LSE, 3× Nasdaq 100 ETP) / `IBTL` (LSE, iShares $ Treasury 20+yr UCITS — **1× not 3×**) / `IBTM` (LSE, iShares $ Treasury 7-10yr UCITS). Trend signals: `QQQ` + `TLT` from EODHD (US-listed; signal-only, never traded). Original notebook universe: TQQQ / TMF / IEF (US-domiciled, PRIIPs-blocked from UK retail accounts per the M2-IB.6.1 wire finding). | F0 | 1d (T+1 open) | active research, DSL-formalised | **Phase 1 (M2-IB.6)** — per [ADR-043](../decisions/DECISIONS.md#adr-043--phase-1-strategy-switch-triple_lev_sma_filter_dsl-a3-replaces-tkan_v4_momentum_timing-a2) + [ADR-047](../decisions/DECISIONS.md#adr-047--priips-compliant-universe-for-phase-1-a3-strategy-refines-adr-043) | 5–10% of total NAV, cap 10% ([ADR-020](../decisions/DECISIONS.md#adr-020--phase-1-nav-slice-510-of-total-cap-10)) |
...
- **v0.3 (2026-05-03)** — `triple_lev_sma_filter_dsl` row "universe" column updated per [ADR-047](../decisions/DECISIONS.md#adr-047--priips-compliant-universe-for-phase-1-a3-strategy-refines-adr-043) — Phase 1 deployment universe is **QQL3 / IBTL / IBTM on LSE** (UK-listed PRIIPs-compliant analogues), substituting for the original notebook's TQQQ / TMF / IEF (US-domiciled, PRIIPs-blocked from UK retail accounts as observed at the M2-IB.6.1 wire run). Trend signals (QQQ / TLT) unchanged — signal-only consumption from EODHD has no PRIIPs implication. Strategy regime shifts (3×/3× → 3×/1× across the legs) since no UK-listed 3× US-Treasury exists; backtest numbers don't carry forward exactly — re-derived parity envelope at M7.
