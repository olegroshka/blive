...
>
> **Status:** v0.3 DRAFT — M2-entry pass. Will be edited every time an artifact is added, retired, or moved.
>
...
| **KB-10** | [`docs/decisions/DECISIONS.md`](./docs/decisions/DECISIONS.md) | DRAFT v0.6 (2026-04-27) | ADR-001..029 ACCEPTED; ADR-030..033 PROPOSED at M2 entry awaiting operator review. New batch (M2 entry): ADR-030 (per-archetype btest interpreter dispatch; resolves OQ-030; amends ADR-010 prose), ADR-031 (token-bucket rate limiter shape for IB adapters; G3 throttle test substrate), ADR-032 (instrument resolution policy `blive.Instrument` ↔ IB `Contract` / `ConID`; pairs with DD-7), ADR-033 (`AccountUpdate` event shape and 30-s diff-suppressed cadence; pairs with DD-2). | Claude record, Oleg approve | continuous |
...
5. **M2 substrate phase** — 2026-04-27: ADR-030..033 PROPOSED; DD-7 / DD-2 / KB-8 DRAFT; KB-2 / KB-3 v0.1.1 review pass (no amendments; STABLE flip deferred). Awaiting operator review before flipping ADR statuses to ACCEPTED and starting code.
6. **M2 execution (post-confirmation)** — IB read-side adapter + operational foundation per [TASK_REGISTRY](./TASK_REGISTRY.md) M2; first code module is the [ADR-031](./docs/decisions/DECISIONS.md#adr-031--token-bucket-rate-limiter-shape-for-ib-adapters) token-bucket rate limiter (no IB wire dependency; G3 throttle test substrate). Conditioned on G2 operator-side prereqs (IB Paper credentials, Docker host) and PROPOSED ADR confirmation. [NEXT_PROMPT.md](./NEXT_PROMPT.md) v0.3 drafted at M1 close.
